...
GLOBAL	error0, error1, pidStat1, PidInitalize, PidMain, pidOut0, pidOut1, pidOut2	
EXTRN	AARGB0, AARGB1, AARGB2, AARGB3, FXM1616U, FXD2416U, _24_BitAdd, _24_bit_sub		
EXTRN	BARGB0, BARGB1, BARGB2, BARGB3
	
#define	aErr1Lim		0x00	;accumulative error limits (255d)
#define	aErr2Lim		0xAF
...
psect udata_acs
#ifdef	pid_100
percent_err:	ds	1			;8-bit error input, 0 - 100% (0 - 100d)
percent_out:	ds	1			;8-bit output, 0 - 100% (0 - 100d)
#endif	

derivCount:	ds	1			;This value determins how many times the Derivative term is 							;calculated based on each Integral term.
pidOut0:	ds	1			;24-bit Final Result of PID for the "Plant"
pidOut1:	ds	1
pidOut2:	ds	1
error0:		ds	1			;16-bit error, passed to the PID
error1:		ds	1			
a_Error0:	ds	1			;24-bit accumulated error, used for Integral term
a_Error1:	ds	1			
a_Error2:	ds	1
p_Error0:	ds	1			;16-bit previous error, used for Derivative term
p_Error1:	ds	1			
d_Error0:	ds	1			;16-bit delta error (error - previous error)
d_Error1:	ds	1
prop0:		ds	1			;24-bit proportional value 
prop1:		ds	1
prop2:		ds	1
integ0:		ds	1
integ1:		ds	1    
integ2:		ds	1
deriv0:		ds  1			;24-bit Derivative value 
deriv1:		ds	1
deriv2:		ds	1
kp:		ds	1			;8-bit proportional Gain
ki:		ds	1			;8-bit integral Gain
kd:		ds	1			;8-bit derivative Gain
pidStat1:	ds	1			;PID bit-status register
pidStat2:	ds	1			;PID bit-status register2
tempReg:	ds	1			;temporary register
...
psect PID_Code, class = CODE			;start PID code here
;org 0x200
;***********************************************************************;
; Function: PidInit                                                		;
;                                                                       ;
...
PidInitalize:
	...
	movlw	1					;10 x 16, Kp, Ki & Kd are 8-bit vlaues that cannot exceed 255
	movwf	kp, A						;Enter the PID gains scaled by a factor of 16, max = 255
	...
	movlw	10
	movwf	derivCount, A				;derivative action = TMR1H:TMR1L * derivCount
	bcf	pidStat1,err_z, A			;start w/error not equal to zero
	bsf	pidStat1,a_err_z, A		;start w/a_error equal to zero
	bsf	pidStat2,d_err_z, A		;start w/d_error equal to zero
	bsf	pidStat1,p_err_sign, A		;start w/ previous error = positive
	bsf	pidStat1,a_err_sign, A		;start w/ accumulated error = positive	
	
	bcf	PIR1,0				;clear T1 flag
	bsf	PIE1,0				;enable T1 interrupt
	
	movlw	00000001B				;configure T1 for Timer operation from Fosc/4
	movwf	T1CON, A		
...
;***********************************************************************;
PidMain:
	bcf PIE1, 0			;disable T1 interrupt
...
#endif			
	movlw	0
	cpfseq	error0, A				;Is error0 = 00 ?
	bra		call_pid_terms		;NO, done checking
	
	cpfseq	error1, A				;YES, Is error1 = 00 ?
	bra		call_pid_terms		;NO, start proportional term
	bsf		pidStat1,err_z, A		;YES, set error zero flag
	bsf		PIE1,0			;enable T1 interrupt
	return						;return back to the main application code
...
;***********************************************************************;
Integral:		
	btfsc	pidStat1,a_err_z, A	;Is a_error = 0
	...
	return						;return
integral_zero:
	clrf	integ0	, A			;a_error = 0, clear Integral term
	clrf	integ1, A
	clrf	integ2, A
	return
...
;***********************************************************************;
Derivative:
	btfsc	pidStat2,d_err_z, A	;Is d_error = 0?
	...
	return						;return
derivative_zero:
	clrf	deriv0, A		;d_error = 0, clear Derivative term
	clrf	deriv1, A
	clrf	deriv2, A
	return
...
integ_mag:							;integ > prop
	bcf	pidStat1,pid_sign, A		;PID result is negative
	btfsc	pidStat1,a_err_sign, A			
	bsf	pidStat1,pid_sign, A		;PID result is positive
	bra 	add_derivative			;(Prop + Integ) + derivative
	
prop_mag:							;integ < prop
	bcf	pidStat1,pid_sign, A		;PID result is negative
	btfsc	pidStat1,err_sign, A			
	bsf	pidStat1,pid_sign, A		;PID result is positive
...
add_derivative:
	...
	movlw	11000000B				;prepare for sign check of bits 7 & 6
	andwf	tempReg, A, F
	
	movf	tempReg, A, F				;check error sign & a_error sign bits
	sublw	0x00
	btfsc	STATUS, 0
...
find_mag_sub_d:
	...
	btfss	pidStat1,mag, A			;which is greater in magnitude ?
...
deriv_mag:							;derivative term > part pid term
	bcf		pidStat1,pid_sign, A		;PID result is negative
	btfsc		pidStat1,d_err_sign, A			
	bsf		pidStat1,pid_sign, A		;PID result is positive
		
scale_down:
	;clrf	BARGB0, A					;(Prop + Integ + Deriv) / 16 = FINAL PID RESULT to plant
	;movlw	0x10	
	;movwf	BARGB1, A
	;call	FXD2416U
...
GetA_Error:				
	...
	bcf	pidStat1,a_err_sign, A		;aargb, make sign same as error, a_error is negative
	btfsc	pidStat1,err_sign, A			
	bsf	pidStat1,a_err_sign, A		;a_error is positive
	
a_err_zero:
	bcf	pidStat1,a_err_z, A		;clear a_error zero flag
	movlw	0
	cpfseq	AARGB0, A					;is byte 0 = 00
	bra	chk_a_err_limit			;NO, done checking
	
	cpfseq	AARGB1, A					;is byte 1 = 00
	bra	chk_a_err_limit			;NO, done checking
	
	cpfseq	AARGB2, A					;is byte 2 = 00
	bra	chk_a_err_limit			;NO, done checking
	bsf	pidStat1,a_err_z, A		;YES, set zero flag
...
chk_a_err_limit:
	...
	cpfseq	a_Error0, A				;Is a_Error0 > 0 ??, if yes limit has been exceeded
	bra		restore_limit			;YES, restore limit value
	
	cpfseq	a_Error1, A				;Is a_Error1 = 0 ??, if yes, limit not exceeded
	bra		chk_a_Error1			;NO
	return							;YES

chk_a_Error1:	
	movlw	aErr1Lim			
	cpfsgt	a_Error1, A				;Is a_Error1 > aErr1Lim??
	bra	equal_value				;NO, check for a_Error1 = aErr1Lim ?
	bra	restore_limit			;YES, restore limit value	
equal_value:
	cpfseq	a_Error1, A				;a_Error1 = aErr1Lim?
	return							;no, done checking a_error
chk_a_Error2:	
	movlw	aErr2Lim				;Yes, a_Error1 = aErr1Lim
	cpfsgt	a_Error2, A				;Is a_Error2 > aErr2Lim ??
	return							;NO, return to mainline code
	
restore_limit:
	clrf	a_Error0, A				;YES, a_error limit has been exceeded
	movlw	aErr1Lim				
	movwf	a_Error1, A		
	movlw	aErr2Lim
	movwf	a_Error2, A	
...
;***********************************************************************;
GetDeltaError:
	clrf	AARGB0, A					;load error and p_error
	movff	error0,AARGB1		
	movff	error1,AARGB2
	clrf	BARGB0, A
	...
	movf	pidStat1, A, W				;pidStat1 ---> tempReg
	movwf	tempReg, A					;prepare for sign check of bits 4 & 2
	movlw	00010100B
	andwf	tempReg, F
		
	movf	tempReg, A, W				;check error sign & a_error sign bits
	sublw	0x00
	btfsc	STATUS, 2		
...
p_err_neg:
	call	MagAndSub			
	bcf		pidStat1,d_err_sign, A		;d_error is negative
	btfsc	pidStat1,p_err_sign, A		;make d_error sign same as p_error sign
	bsf		pidStat1,d_err_sign, A		;d_error is positive
...
p_err_add:
	...
	bcf		pidStat1,d_err_sign, A		;d_error is negative
	btfsc	pidStat1,err_sign, A		;make d_error sign same as error sign
	bsf		pidStat1,d_err_sign, A		;d_error is positive
...
d_error_zero_chk:
	...
	bcf		pidStat1,p_err_sign, A		;make p_error negative
	btfsc	pidStat1,err_sign, A		;make p_error the same sign as error
	bsf		pidStat1,p_err_sign, A		;make p_error positive	
	
	bcf		pidStat2,d_err_z, A		;clear delta error zero bit	
	movlw	0
	cpfseq	d_Error0, A				;is d_error0 = 00
	return							;NO, done checking
	
	cpfseq	d_Error1, A				;YES, is d_error1 = 00
	return							;NO, done checking
	bsf		pidStat2,d_err_z, A		;set delta error zero bit
...
SpecSign:
	movff	pidStat1,tempReg		;pidStat1 ---> tempReg
	movlw	00001100B				;prepare for sign check of bits 3 & 2
	andwf	tempReg, F
...
MagAndSub:			
	movf	BARGB0, A, W
	subwf	AARGB0, A, W				;AARGB0 - BARGB0 --> W
	btfsc	STATUS,2				;= zero ?
	bra		check_1					;YES
	btfsc	STATUS,0				;borrow ?
	bra		aargb_big				;AARGB0 > BARGB0, no borrow
	bra		bargb_big				;BARGB0 > AARGB0, borrow
check_1:
	movf	BARGB1, A, W
	subwf	AARGB1, A, W				;AARGB1 - BARGB1 --> W
	btfsc	STATUS,2				;= zero ?
	bra		check_2					;YES
	btfsc	STATUS,0				;borrow ?
...
check_2:
	movf	BARGB2, A, W				;AARGB2 - BARGB2 --> W
	subwf	AARGB2, A, W
	btfsc	STATUS,0				;borrow ?
...
aargb_big:
	call	_24_bit_sub		
	bsf		pidStat1,mag, A			;AARGB is greater in magnitude
	return
...
bargb_big:
	...
	bcf		pidStat1,mag, A			;BARGB is greater in magnitude
	return	
...
derivative_ready:
	...
	movwf	derivCount, A					;delta error = TMR1H:TMR1L * derivCount
